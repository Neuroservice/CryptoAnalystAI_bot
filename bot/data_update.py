...
def get_top_projects_by_capitalization(session, project_type, tickers, top_n_tickers=5, top_n_other=10):
    top_ticker_projects = (
        ...
        .filter(Project.project_name == project_type, Project.coin_name.in_(tickers))
...
async def update_agent_answers(session):
    ...
    for answer in outdated_answers:
        ...
        projects, tokenomics_data_list = await get_project_and_tokenomics(session, project.project_name, project.coin_name)
        top_projects = sorted(
